  ...
  return (
    <section className="studio-canvas border-b border-studio-line py-[var(--studio-section-space-compact)]">
      <Container size="wide">
        ...
        <ol className="mt-9 grid border-y border-studio-line lg:grid-cols-3">
          {idealClients.profiles.map((profile) => (
            <li
              className="border-t border-studio-line py-6 first:border-t-0 lg:border-l lg:border-t-0 lg:px-6 lg:first:border-l-0 lg:first:pl-0 lg:last:pr-0"
              key={profile.index}
            >
              <p className="type-label text-studio-greenBright">{profile.index}</p>
              <h3 className="type-subheading mt-3 text-studio-text">{profile.title}</h3>
              <p className="type-small mt-3 text-studio-muted">{profile.description}</p>
            </li>
          ))}
        </ol>
      </Container>
